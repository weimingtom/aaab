	...
	CMSAPP.addkehu = {
		...
		ajaxfrom: function(){
			$('#publishForm').submit(function(){				
				$.ajax({
					type: "POST",
					url: "?c=getdata&a=database",
					data: $('#publishForm').serialize(true),
					cache: false,
					beforeSend: function(){ $('#apptable').slideToggle(100);$('#wait').show().html('<label style="color:red;font-size:16px;">正在获取数据至数据库,请稍候...</label>'); },
...
<div id="wait" style="font-size: 16px; display: none; margin-bottom: 5px;"></div>
	<div id="apptable">
		...
		<tr>
		  ...
		  <td align="left"><input type="submit" class="inputsubmit" name="Submit" value="  抓取数据  " />
