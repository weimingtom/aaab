...
$.fn.ShowTime = function(options){
	...
	{		
		...
		var sel = "<select style='width:50px;' name='inMonth' id='inMonth' bj='cBj'>";
		vMonth = ''+vMonth+''
		vMonth = vMonth.replace(/0?/, '');
		
		for(var im = 1; im<= 12; im++) {
			if(parseInt(vMonth) == parseInt(im))
			{
				sel += "<option selected = 'selected'>"+ (im <= 9 ? '0' + im : im) +"</option>";
			} else {
				sel += "<option>"+ (im <= 9 ? '0' + im : im) +"</option>";
			}
		}		
		sel += "</select>"
		
		var cstr = '';
		cstr += "<table border='0' cellspacing='0' cellpadding='0' width=220>";
		cstr += "<tr><td>";
		cstr += "<table border='0' cellspacing='0' cellpadding='0'>";//<input name='inMonth' type='text' id='inMonth' value='"+ vMonth +"' maxlength='2' bj='cBj' />
		...
		cstr += "<td width='20'>年</td><td width='25' height='30'>" +sel+ "</td>";
		cstr += "<td width='20'>月</td><td width='20'><span id='yadd' bj='cBj'>&gt;&gt;</span></td><td width='13'>&nbsp;</td><td><input type=submit name=Submit id=submitselect value=选中 ipt='"+ vipt +"' /></td>";
		cstr += "</tr></table>";
		cstr += "</td></tr>";
...
//		}
		...
		cstr = '';
		$("#submitselect").bind('click', function(){$('#'+vipt).val($('#inYear').val() + '-' + $('#inMonth').val()); $("#carlendar_div").css("display", "none");});
		//手动输入框的年份和月份框的样式
	...
	{		
		 ...
		 if((keyStr >= 48 && keyStr <= 57) || (keyStr >= 96 && keyStr <= 105))
		 {
			...
			{
				var cipt = $('#submitselect').attr('ipt');//$("#carlendar_div a").eq(0).attr("ipt");
	...
	{		
		...
		var lipt = $('#submitselect').attr('ipt');//$("#carlendar_div a").eq(0).attr("ipt");
		if(mVal <= 0)
		{
			mVal = "12";
			yVal = yVal - 1;
	...
	{	
		...
		var dVal = $("#carlendar_div #cur").text();
		var aipt = $('#submitselect').attr('ipt');//$("#carlendar_div a").eq(0).attr("ipt");
