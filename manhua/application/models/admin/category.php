...
class admin_category
{
	...
	{		
        ...
        foreach ($arr as $val)
        {
             $data[] = array(
             	...
             	'ismenu' => $val['ismenu'] ? '是': '否',
             	'catestat' => $this->db->result_first("SELECT COUNT(*) FROM god_novel WHERE categoryId = '". $val['cate_id'] ."' "),
             	'level'	=> $val['level'],
    ...
    */
	function _init_tree ()
	{	
		$arr = $this->db->fetch_all("SELECT id AS cate_id,store_id,cate_name,ismenu,menulink,parent_id,sort_order,`time` FROM `mh_categories` WHERE store_id = 0 ");
