...
class LetterList
{
	...
	var $lettervalue;
	//php5构造函数
	function __construct($letterconfig = array(), $lettervalue)
	{
		global $dsql;
		//$this->FreeID = $fid;
		...
		*/
		...
		$this->Fields['title'] = str_replace("{letter}", $this->lettervalue, $this->FLInfos['showtitle']);
		$this->Fields['position'] = isset($this->FLInfos['title']) ? $this->FLInfos['title'] : '';
		$this->Fields['keywords'] = $this->FLInfos['keywords'];
		$this->Fields['description'] = $this->FLInfos['description'];
		$this->Fields['letter'] = $lettervalue;
		$channelid = $this->ListObj->GetAtt('channel');
		if(!empty($channelid))
		{
	...
	function CountRecord()
	{
		...
		if($this->TotalResult==-1)
		{
			...
			$templettervalue = strtolower($this->lettervalue);
			$addSql .= " AND titleenglish LIKE '{$templettervalue}%'";
		...
		$this->Fields['totalnum'] = $this->TotalResult;
	...
	function MakeHtml($startpage=1,$makepagesize=0)
	{
		...
		{
			$listdir = str_replace('{cmspath}',$GLOBALS['cfg_cmspath'],$this->FLInfos['listdir']);
			$murl = str_replace('{listdir}',$listdir,$this->FLInfos['namerule']);
			$murl = str_replace('{letter}',$this->lettervalue,$murl);
			$murl = str_replace('_{page}','',$murl);
			$indexfile = $GLOBALS['cfg_basedir'].$murl;
			$murl = ereg_replace("/{1,}","/",$murl);
			echo "复制：$firstFile 为 ".$murl." <br/>";
	...
	function GetMakeFileRule()
	{
		...
		$okfile = str_replace('{letter}',$this->lettervalue,$okfile);
	...
	function GetList($limitstart,$ismake=1)
	{
		...
		$templettervalue = strtolower($this->lettervalue);
		$orwhere .= " AND arc.titleenglish LIKE '{$templettervalue}%'";
