...
class PageClass
{
	...
	private $page_i;           //起始页
	private $page_ub;          //结束页
	private $index_url;        //首页(特殊用途，比如可以写目录)
	var $page_limit;
	
	function __construct($myde_count=0, $myde_size=1,$myde_page=1,$page_url,$index_url="")//构造函数
	{	
	...
	private function myde_home() //首页
	{
		if($this -> myde_page != 1){
			...
			return "    <li class=\"page_a\"><a href=\"".($this -> index_url <>"" ? $this -> index_url : $this -> page_replace(1))."\"  title=\"首页\" >首页</a></li>\n";
	...
	private function myde_prev() //上一页
	{
		if($this -> myde_page != 1){
			...
			return "    <li class=\"page_a\"><a href=\"".(($this -> index_url <>"" && $this->myde_page-1 ==1) ? $this -> index_url : $this -> page_replace($this->myde_page-1)) ."\"  title=\"上一页\" >上一页</a></li>\n";
		...
		for($page_for_i = $this -> page_i;$page_for_i <= $this -> page_ub; $page_for_i++){
				if($this -> myde_page == $page_for_i){
					...
					if($page_for_i==1 && $this -> index_url <>""){
						$str .= "    <li class=\"page_a\"><a href=\"".$this -> index_url."\" title=\"第".$page_for_i."页\">";
