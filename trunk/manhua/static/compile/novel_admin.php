...
<? foreach((array)$categorys as $val) {?>
...
<div style="text-align:right"><?=$multipage?></div>
...
<? foreach((array)$novels as $val) {?>
<tr>
  <td><input type="checkbox" name="P[]" value="<?=$val['novelId']?>" /></td>
  <td><?=$val['novelId']?></td>
  <td><img src="<?=$val['picture']?>" width="50" onmouseover="this.width=110" onmouseout="this.width=50" /></td>
  <td><a href="index-detail-<?=$val['novelId']?>.htm" target="_blank"><?=$val['novelName']?></a></td>
  <td><?=$val['totals']?></td>
  <td><?=$val['views']?></td>
  <td><?=$val['author']?></td>
  <td><?=$val['createdTime2']?></td>
  <td>编辑&nbsp;&nbsp;<a href="javascript:if(confirm('确定是否要删除吗?')){location.href='admin.php?c=novel&a=delete&novelId=<?=$val['novelId']?>';}">删除</a></td>
</tr>
<? } ?>
<tr>
  <td colspan="9">
...
<input name="" type="submit" value="确定移动" />
  ...
  <td width="79%" style="text-align:right"><?=$multipage?></td>
  </tr>
  </table>
  </td>
</tr>
