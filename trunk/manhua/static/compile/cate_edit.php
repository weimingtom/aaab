...
$(function(){
	...
	CMSAPP.addkehu = {
		...
		init: function(){
			...
			var cateid = '<?=$info['parent_id']?>';
			
			if(cateid != '' && cateid > 0)$('#parent_id').val('<?=$info['parent_id']?>');	
		}
	}
...
<span><a href="admin.php?c=cate&a=add">添加分类</a></span>
<span>编辑分类</span>
</div>
...
<div class="c1">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="c1content">
           ...
           <tr>
            ...
            <td><input name="cate_name" type="text" class="w01" id="cate_name" value="<?=$info['cate_name']?>" /></td>
          </tr>
           <tr>
             <td align="right">作为导航：</td>
             <td><label>
               <input name="ismenu" type="checkbox" id="ismenu" value="1" <? if($info['ismenu'] == 1) { ?> checked="checked"<? } ?> />
             </label></td>
           </tr>
           <tr>
             <td align="right">分类链接：</td>
             <td><input name="menulink" type="text" class="w01" id="menulink" value="<?=$info['menulink']?>" /></td>
           </tr>
          <tr>
            <td width="10%" align="right">分类排序：</td>
            <td><input name="sort_order" type="text" class="w01" id="sort_order" value="<?=$info['sort_order']?>" /></td>
          </tr>
          
          <tr>
            <td align="right">备注：</td>
            <td><textarea name="content" cols="45" rows="3" id="content"><?=$info['content']?></textarea></td>
          </tr>
          <tr>
            <td align="right">&nbsp;</td>
            <td><input class="w02" name="input" type="submit" value="编 辑" />
            &nbsp;&nbsp;<input class="w02" name="input" type="reset" value="重 填" />
            <input type="hidden" name="action" value="edit" />
			<input type="hidden" name="cateid" value="<?=$info['cateid']?>" />			</td>
            </tr>
        </table>
