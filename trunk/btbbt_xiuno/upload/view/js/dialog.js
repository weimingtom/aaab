/* Dialog base on JQuery1.4
*
* Copyright (C) 2012 A.Xiuno(axiuno@gmail.com)
* Support: http://www.xiuno.com/
* Version: 1.0
* Last Modified: 2012/2/23
...
if(!$.fn.dialog) {
	...
	$.dialog = function(div, settings) {
		...
		this.init = function() {
			...
			// 构造对话框
			if($('#overlay').length == 0) $('<div class="overlay" id="overlay" ref="0" ></div>').hide().appendTo('body').height($(document).height());
		...
		this.set = function(settings) {
			...
			if(isset(settings.title))  this.set_title(settings.title);
		...
		*/
		this.set_position = function(position) {
			...
			// 居中 'center'
				...
				var v_top = ($(window).height() / 2 - $(_div).height() / 2) + $(document).scrollTop();
				var v_left = ($(window).width() / 2 - $(_div).width() / 2);
				if(v_top < 0) v_top = 10;
				if(v_top > 100) v_top -= 100;
				$(_div).css({ top: v_top, left: v_left });
		...
		/*// 钩子：关闭窗口的时候，移除掉注册的对象
		this.register_remove_object = function(obj) {
			_register_remove_object[_register_remove_object.length] = obj;
		}*/
		
		/*// 删除注册的对象
		this.remove_register_object = function() {
			if(_register_remove_object.length > 0) {
				for(var i=0; i<_register_remove_object.length; i++) {
					$(_register_remove_object[i]).remove();
				}
			}
		}*/
