...
class mod_control extends common_control {
		...
		if(!$this->form_submit()) {
			...
			foreach($tidarr as &$v) {			// 此处也得用 &
				...
				$this->modlog->create(array(
					...
					'pid'=>0,
					'subject'=>$thread['subject'],
					'credits'=> $rank == 0 ? 0 - $this->conf['credits_policy_digest_'.$rank] : $this->conf['credits_policy_digest_'.$rank],
					'dateline'=>$_SERVER['time'],
	...
	public function on_move() {
		...
		if(!$this->form_submit()) {
			...
			$movethreads = count($tidarr);
			$this->view->assign('movethreads', $movethreads);
