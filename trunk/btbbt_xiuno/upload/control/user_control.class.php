...
class user_control extends common_control {
	...
	public function on_login() {
		...
		if($this->form_submit()) {
			...
			$uid = $this->user->get_uid_by_email($user['email']);
			$userdb = $this->user->get($uid);
			if(empty($userdb)) {
				$userdb = $this->user->get_user_by_username($user['email']);
				if(empty($userdb)) {
					$error['email'] = '用户名/Email 不存在';
					log::write('EMAIL不存在:'.$user['email'], 'login.php');
					$this->message($error);
				}
			}
			
			if(!$this->user->verify_password($user['password'], $userdb['password'], $userdb['salt'])) {
				$error['password'] = '密码错误!';
				$log_password = '******'.substr($user['password'], 6);
				log::write("密码错误：$user[email] - $log_password", 'login.php');
				$this->message($error);
			}
	...
	/*
	// 给第三方插件提供:同步登陆
	public function on_rsynclogin() {
		$email = core::gpc('email', 'G');
		$password = core::gpc('password', 'G');
		...
		if(empty($userdb)) {
			...
			if(empty($userdb)) {
				log::write('rsync EMAIL不存在:'.$email, 'login.php');
				exit('EMAIL不存在:'.$user['email']);
			}
		}
		if(!$this->user->verify_password($password, $userdb['password'], $userdb['salt'])) {
			$log_password = '******'.substr($password, 6);
			log::write("rsync 密码错误：$email - $log_password", 'login.php');
			exit('密码错误');
		}
		...
		// 更新在线列表
		$this->init_online_create();
		//echo 'alert("ok")';
	}
	
	// 给第三方插件提供:同步退出
	public function on_rsynclogout() {
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		misc::set_cookie($this->conf['cookiepre'].'auth', '', 0, '/');
		//echo 'alert("ok")';
	}
	*/
	...
	public function on_create() {
		...
		if($this->form_submit()) {
			...
			$error['username_exists'] = $this->user->check_username_exists($user['username']);
			...
			if(misc::values_empty($error)) {
				$error = array();
				$user['groupid'] = $this->conf['reg_email_on'] ? 6 : 11;
				$uid = $this->user->create($user);
				if($uid) {
					// 发送激活邮件
					if($this->conf['reg_email_on']) {
						try {
							$this->send_active_mail($uid, $user['username'], $user['email'], $error);	// $error['email_smtp_url']
						} catch(Exception $e) {
