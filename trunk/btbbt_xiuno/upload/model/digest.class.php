...
class digest extends base_model {
	...
	*/
	...
	public function get_newlist($limit) {
		$limit += $limit;
		$digestlist = $this->index_fetch(array(), array('digestid'=>-1), 0, $limit);
		$misc = $_SERVER['miscarr'];
		$tids = array();
		foreach($digestlist as &$digest) {
			if(in_array($digest['tid'], $tids)) {
				continue;
			} else {
				array_push($tids, $digest['tid']);
			}
			$digest['forumname'] = $misc['forum'][$digest['fid']];
		}
		$digestlist = array_slice($digestlist, 0, 10);
		return $digestlist;
	}
