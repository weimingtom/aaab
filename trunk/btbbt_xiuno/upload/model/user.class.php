...
class user extends base_model{
	...
	public function create($arr) {
		...
		$arr['salt'] = rand(100000, 999999);
		$arr['password'] = $this->md5_md5($arr['password'], $arr['salt']);
	...
	*/
	public function encrypt_auth($uid, $username, $password, $groupid) {
		...
		// 所有项目中，不允许有\t，否则可能会被伪造
		$xn_auth = encrypt("$uid	$username	$groupid	$password	$ip", $key);
		return $xn_auth;
	}
	...
	public function decrypt_auth($xn_auth) {
		...
		$return =  array('uid'=>0, 'username'=>'', 'groupid'=>0, 'password'=>'', 'ip_right'=>FALSE);
		if(!$s) {
			return $return;
		}
		$arr = explode("\t", $s);
