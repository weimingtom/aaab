...
class core {
	...
	*/
	...
	public static function init_supevar() {
		...
		!isset($_SERVER['REQUEST_URI']) && self::fix_iis_request();
	...
	public static function run() {
		...
		if(!is_file($objfile) || DEBUG) {
			...
			if(empty($controlfile)) {
				throw new Exception("{$control}_control.class.php 不存在。");
			}
			...
			$s = preg_replace('#\t*\/\/\s*hook\s+([^\s]+)#ies', "core::process_hook('\\1')", $s);
		...
		if(@include $objfile) {
			...
			if(method_exists($newcontrol, $onaction)) {
				...
				throw new Exception('方法未实现:'.$action);
			}
		} else {
			throw new Exception("control $newcontrol does not exists.");
		}
