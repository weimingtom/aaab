...
class SpaceAction extends Action
{
    ...
    public function index()
    {
       ...
       $bcdata = M ( 'weibo_bc' )->where ( 'uid=' . $this->uid )->order('bc_id DESC')->findPage(20);
       $this->assign ($bcdata);
    ...
    //  by 仿知美二次开发 QQ:2451302968 用户分类页
    function board()
    {
        //by 仿知美二次开发 qq:2451302968
         $intbc_id = intval( $_GET['bc_id'] );
		 
         $bcdata = M ( 'weibo_bc' )->where ("uid = '".$this->uid."' AND bc_id<> '".$intbc_id."'")->order('bc_id DESC')->findall();
		...
		//增加分类选择，针对快速建图格
		$ac_id= M('weibo_bc')->getField('ac_id',$map);
		...
		$fengcount= M('weibo_bc')->getField('fengcount',$map);
		...
		$row = $row?$row:20;
		$listCount=M('weibo')->where("type in(1,3,4,5) and  isdel=0 and uid = '".$this->uid."' AND bc_id = '".$intbc_id."'")->count();
		$catelog=M('weibo')->where("type in(1,3,4,5) and  isdel=0 and uid = '".$this->uid."' AND bc_id = '".$intbc_id."'")->order('weibo_id DESC')->findPage($row, $listCount);
		$this->assign('list' , $catelog) ;

		
		
		 /* 	$bcdata_sql = "SELECT * FROM {$this->tablePrefix}weibo AS w INNER JOIN {$this->tablePrefix}weibo_bc AS b ON w.uid = b.uid 
    				  WHERE w.weibo_id = 132 limit 1 order b.bc_id )";
         dump($bcdata = M('')->query($bcdata_sql));
	...
	function share()
    {
		...
		if($strType=='fav'){
			...
			$listCount=M('weibo')->where("type in(1,3,4,5) and  isdel=0 and uid = '".$this->uid."' ")->count();
			$catelog=M('weibo')->where("type in(1,3,4,5) and  isdel=0 and uid = '".$this->uid."'")->order($order)->findPage($row, $listCount);
			
    		
    	}else{ 
			$listCount=M('weibo')->where("type = '".$strType."' and  isdel=0 and uid = '".$this->uid."' ")->count();   
			$catelog=M('weibo')->where("type = '".$strType."' and  isdel=0 and uid = '".$this->uid."'")->order($order)->findPage($row, $listCount);
    ...
    function detail(){
		...
		//--------------仿知美二次开发 end -------------------
         ...
         $bc_id=M ( 'weibo' )->field('bc_id')->where('weibo_id='.$intId)->findall();
         ...
         $bcdata = M ( 'weibo_bc' )->where ( 'bc_id = '.$bcbc )->limit(1)->order('bc_id DESC')->findall();
