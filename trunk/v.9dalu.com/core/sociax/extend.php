...
function get_domain($url){
  $pattern = "/[\w-]+\.(com|net|org|gov|cc|biz|info|cn|jp|la|me)(\.(cn|hk))*/";
...
function favislove($id){
  ...
  $myfav = M('fav')->where("favid=$id AND uid=" . $_SESSION['mid'] . "")->findall() ;
  if($myfav){
     return '我';
  }else{
  return '已';
...
//获取分类数
function getBcCount($ac_id) {
   if ($ac_id<>"")
   {
    $bccount=M('weibo_bc')->where('ac_id=' . $ac_id)->count();
    return $bccount;
   }	 
}
...
function getBcuidCount($uid) {
   if ($uid<>"")
   {
    $bcuidcount=M('weibo_bc')->where('uid=' . $uid)->count();
    return $bcuidcount;
   }	 
}
...
function getBname($id) {
    if ($id <> "") {
        $bname = M('weibo')->where('weibo_id=' . $id)->findall();
...
function getCcCount($bc_id) {
   if ($bc_id<>"")
   {
    $cccount=M('weibo')->where( 'type in(1,3,4,5) and isdel=0 and bc_id=' . $bc_id)->count();
    return $cccount;
   }	 
}
//自定义提取微博图片
function getCcpic($bc_id) {
   if ($bc_id<>"")
   {
      ...
      //$map['type']=5;
      ////$map['transpond_id']=0;
     $ccimages=M('weibo')->where("type in(1,3,4,5)  and isdel=0 and bc_id='".$bc_id."'")->limit(9)->order('weibo_id DESC')->findall();
        ...
        foreach ($ccimages as $somedata)
        {
           ...
           // $st_arr['uid']=$somedata['uid'];
            ...
            if($st_transpond_id <> '0')
                {
               // echo $st_transpond_id;
                
             
						 $tr_id=M('weibo')->where('weibo_id='.$st_transpond_id)->limit(9)->order('weibo_id DESC')->findall();
            ...
            // $st_arr['content']=$somedata['content']; 
      
           
          // $url = U('home/Space/detail', array('id' => $st_weibo_id));
...
function getCcpic_updown($bc_id,$weibo_id,$left_right) {
   if ($bc_id<>"")
   {
      ...
      ////$map['transpond_id']=0;
	  	switch ($left_right){
	  	case '1':  
     	$ccimages=M('weibo')->where("type in(1,3,4,5)  and isdel=0 and bc_id='".$bc_id."' and weibo_id<'".$weibo_id."'")->limit(1)->order('weibo_id DESC')->findall();
		$jquery='jquery-righttshow';
		break;
		
		case '2':  
     	
		$ccimages=M('weibo')->where("type in(1,3,4,5)  and isdel=0 and bc_id='".$bc_id."' and weibo_id>'".$weibo_id."'")->limit(1)->order('weibo_id ASC')->findall();
           ...
           // $st_arr['uid']=$somedata['uid'];
            ...
            if($st_transpond_id <> '0')
               ...
               // echo $st_transpond_id;
						 ...
						 $tr_id=M('weibo')->where("weibo_id='".$st_transpond_id."'")->limit(1)->order('weibo_id DESC')->findall();
						
						  foreach ($tr_id as $tr)
						  {
						   $trimg =unserialize($tr['type_data']);
						   $type =$tr['type'];
						   $weibo_id =$tr['weibo_id'];
						   
								   if($type == 1)
								   {
										$st_arr.='<a id="'.$jquery.'" href="'.U("home/Space/detail",array("id"=>$weibo_id)).'"></a>';
							 ...
							 if($type == 1)
							   {
									  ...
									  $st_arr.='<a id="'.$jquery.'" href="'.U("home/Space/detail",array("id"=>$st_weibo_id)).'"></a>';  
							 }else if($type == 5)
							 {
...
function getUserSpace($uid,$class,$target,$text, $verified = true) {
	...
	//增加悬浮卡片
	//return "<a href='{$url}' class='{$class}' target='{$target}' rel=\"_widget_user\" user_id=\"".$uid."\" id='user_".$uid."'>$text</a>" . $user_verified_icon;
...
function format($content,$url=false){
	if($url){
		$content = preg_replace('/((?:https?|mailto):\/\/(?:www\.)?(?:[a-zA-Z0-9][a-zA-Z0-9\-]*\.)?[a-zA-Z0-9][a-zA-Z0-9\-]*(?:\.[a-zA-Z]+)+(?:\/[^\x{4e00}-\x{9fa5}\s<\'\"“”‘’]*)?)/u', '<a href="\1" target="_blank">\1</a>\2', $content);
	}
...
function formatComment($content,$url=false){
	if($url){
		$content = preg_replace('/((?:https?|mailto):\/\/(?:www\.)?(?:[a-zA-Z0-9][a-zA-Z0-9\-]*\.)?[a-zA-Z0-9][a-zA-Z0-9\-]*(?:\.[a-zA-Z0-9]+)+(?:\/[^\x{4e00}-\x{9fa5}\s<\'\"“”‘’]*)?)/u', '<a href="\1" target="_blank">\1</a>\2', $content);
	}
...
function replaceEmot($data) {
	...
	if($info) {
		return preg_replace("/\[.+?\]/i","<img src='".__THEME__."/images/expression/miniblog/".$info['filename']."' />",$data[0]);
	}else {
		return $data[0];
	}
}
...
function getFrom($type, $type_data) {
	if( $type_data['app_name'] && $app_alias = getAppAlias($type_data['app_name']) ) {
		...
		$html = '<a href="'.U("home/Caiji/tools").'">采集工具</a>';
	}
	return $html;
}
