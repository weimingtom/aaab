...
class OAuthToken { 
    ...
    function to_string() { 
        return "oauth_token=" . 
            QQOAuthUtil::urlencode_rfc3986($this->key) .
            "&oauth_token_secret=" . 
            QQOAuthUtil::urlencode_rfc3986($this->secret);
    } 
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod { 
    ...
    public function build_signature($request, $consumer, $token) { 
		...
		$key_parts = QQOAuthUtil::urlencode_rfc3986($key_parts);
...
class OAuthSignatureMethod_PLAINTEXT extends OAuthSignatureMethod { 
    ...
    public function build_signature($request, $consumer, $token) { 
        $sig = array( 
            QQOAuthUtil::urlencode_rfc3986($consumer->secret)
        ); 

        if ($token) { 
            array_push($sig, QQOAuthUtil::urlencode_rfc3986($token->secret));
        } else { 
            array_push($sig, ''); 
        } 
        ...
        return QQOAuthUtil::urlencode_rfc3986($raw);
    } 
} 
...
class QQOAuthRequest {
    ...
    public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) { 
        ...
        if (!$parameters) { 
            ...
            $request_headers = QQOAuthUtil::get_headers();

            // Parse the query-string to find GET parameters 
            $parameters = QQOAuthUtil::parse_parameters($_SERVER['QUERY_STRING']);
            ...
            if ($http_method == "POST" 
                ...
                $post_data = QQOAuthUtil::parse_parameters(
            ...
            if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") { 
                $header_parameters = QQOAuthUtil::split_header(
    ...
    public function get_signable_parameters() { 
        ...
        return QQOAuthUtil::build_http_query($params);
    } 
    ...
    public function get_signature_base_string() { 
        ...
        $parts = QQOAuthUtil::urlencode_rfc3986($parts);
    ...
    //echo "multi=" . $multi . '`';
    if( $multi )
    	return QQOAuthUtil::build_http_query_multi($this->parameters);
    else 
        return QQOAuthUtil::build_http_query($this->parameters);
    } 
    ...
    public function to_header() { 
        ...
        foreach ($this->parameters as $k => $v) { 
            ...
            $out .= ',' . 
                QQOAuthUtil::urlencode_rfc3986($k) .
                '="' . 
                QQOAuthUtil::urlencode_rfc3986($v) .
                '"'; 
        } 
        return $out; 
    } 
...
if ( !class_exists('OAuthServer') ) {  //小四加
/** 
 * @ignore 
 */ 
class OAuthServer { 
...
class QQOAuthUtil {
    ...
    public static function urlencode_rfc3986($input) { 
        if (is_array($input)) { 
            return array_map(array('QQOAuthUtil', 'urlencode_rfc3986'), $input);
    ...
    public static function split_header($header, $only_allow_oauth_parameters = true) { 
        ...
        while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) { 
            ...
            if (preg_match('/^oauth_/', $header_name) || !$only_allow_oauth_parameters) { 
                $params[$header_name] = QQOAuthUtil::urldecode_rfc3986($header_content);
            } 
    ...
    public static function parse_parameters( $input ) { 
        ...
        foreach ($pairs as $pair) { 
            ...
            $parameter = QQOAuthUtil::urldecode_rfc3986($split[0]);
            $value = isset($split[1]) ? QQOAuthUtil::urldecode_rfc3986($split[1]) : '';
    ...
    public static function build_http_query_multi($params) { 
        ...
        //$keys = QQOAuthUtil::urlencode_rfc3986(array_keys($params));
        //$values = QQOAuthUtil::urlencode_rfc3986(array_values($params));
...
class QqWeiboOAuth {
   ...
   function requestTokenURL() { return 'https://open.t.qq.com/cgi-bin/request_token'; } 
    ...
    function getRequestToken($oauth_callback = NULL) { 
        ...
        $token = QQOAuthUtil::parse_parameters($request);
...
*/
        ...
        $header_array2=array(); 
        if( $multi ) 
        	$header_array2 = array("Content-Type: multipart/form-data; boundary=" . QQOAuthUtil::$boundary , "Expect: ");
