...
/**
 * "等级"勋章
 * by 仿知美二次开发 QQ2451302968
 * web www.fakesay.com
 * 连续10天发原创微博获得此勋章. 用户获得此勋章即为永久拥有.
 * 
 */
class RankMedal extends BaseMedal {
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( intval($user_medal_data['data']['received_time']) <= 0 && $this->__isRank($uid) ) {
	 ...
	 * @return bool
	 */
	private function __isRank($uid, $days = 30) {
		if ($days <= 0) {
			return true;
		}
