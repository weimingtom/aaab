...
switch($action){
	case "add":
		$pageinfo['title']    = "添加友情链接";
		$pageinfo['subtitle'] = "添加";
		$pageinfo['act']      = "act_add";
		$tpl -> display('header');
		$tpl -> display('link_add');
		break;
	case "act_add":
			$arr = array(
			'name'           => str($_POST['name']),
			...
			$pageinfo['title'] = "添加友情链接";
			$tpl -> display('header');
			if($mysql -> insert("-table-link",$arr)){
				msg('添加成功!','?action=');
			}else{
				msg('添加失败!','?action=');
			}
	...
	case "act_edit":
		$id =  numeric($_POST['id']);
		$sql  = "select `id` from `-table-link` where id=".$id;
		...
		if($row){
				...
				$pageinfo['title'] = "修改友情链接";
				$tpl -> display('header');
				if($mysql -> update("-table-link",$arr,' id='.$id)){
	...
	case "edit":
		...
		$tpl -> display('header');
		$sql  = "select * from `-table-link` where id=".$id;
		$query = $mysql -> query($sql);
		$row = $mysql -> fetch_array($query);
		if($row){
			$tpl -> display('link_add');
		...
		break;
	case "del":
		$pageinfo['title'] = "删除友情链接";
		$tpl -> display('header');
		$id = $_POST["id"];
		if(is_array($id)){
			for($i = 0 ; $i < count($id); $i++){
				$sql = "select id from `-table-link` where id=".numeric($id[$i]);
				$query = $mysql -> query($sql);
				$row = $mysql ->fetch_array($query);
				if($row){
					$mysql -> query("delete from `-table-link` where id=".numeric($id[$i]));
				}else{
					msg('记录不存在!','?action=');
				}
			}
			...
			if(!is_numeric($id)){
				...
				$row = $mysql ->fetch_array($query);
				if($row){
					$mysql -> query("delete from `-table-link` where id=".numeric($id));
					msg('删除成功!','?action=');
				}else{
					msg('记录不存在!','?action=');
				}
	...
	case "piliang":
		...
		$target = $_POST["target"];
		$type  = $_POST["type"];
		$pageinfo['title'] = '友情链接批量修改';
		$tpl -> display('header');
		if(is_array($idarr)){
			for($i = 0 ; $i < count($idarr); $i++){
				$sql = "select `id` from `-table-link` where id=".numeric($idarr[$i]);
				$query = $mysql -> query($sql);
				$row = $mysql ->fetch_array($query);
				if($row){
					$arr = array('sort'  => $sort[$i],'target'=>$target[$i],'type'=>$type[$i]);
					$mysql -> update("-table-link",$arr,' id='.$idarr[$i]);
				}else{
