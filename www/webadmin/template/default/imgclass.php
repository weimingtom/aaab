...
<div id="mydecms">
  ...
  <?php $tpl -> display('nav'); ?>
  <div id="location"> 当前位置：<a href="<?php echo $mydecms['weburl'];?>">网站首页</a> → <a href="<?php echo $mydecms['weburl']; ?>/<?php echo $dafenglei['dir'] <> "" ? $dafenglei['dir'] : 'class.php?id='.$dafenglei['id'];?>"><?php echo $dafenglei['name'];?></a> <span class="rss"><a href="<?php echo $mydecms['weburl']; ?>/rss.php?id=<?php echo $dafenglei['id'];?>"><img src="<?php echo $mydecms['weburl'];?>/<?php echo $mydecms['moban'];?>images/rss.png" /></a></span>
...
?>
        <ul class="img-list-ul clear">
          <?php
$sql  = "select `id`,`title`,`type`,`date`,`dafenglei`,`content`,`tag`,`img` from `-table-article` where `type`=5 and `dafenglei` in (".$dafenglei['idstr'].") ";
...
?>
          <li>
            <div class="img-list-suoluetu"><a href="<?php echo $mydecms['weburl']."/".$mydecms['html'].date("Ym",strtotime($row['date'])).'/'.$row['id'].'.html';?>" title="<?php echo  $row['title'];?>"><img src="<?php echo $row['img']<>""? $row['img'] : $mydecms['weburl'].'/'.$mydecms['moban'].'images/suoluetu.gif';?>" onload="if(this.width>150){this.width=150}if(this.height>120){this.height=120}" /></a></div>
            <div class="img-list-right">
              <p class="img-list-title"><a href="<?php echo $mydecms['weburl']."/".$mydecms['html'].date("Ym",strtotime($row['date'])).'/'.$row['id'].'.html';?>"><?php echo  $row['title'];?></a></p>
              <p class="img-list-content"><?php echo  str_len(strip_tags($row['content']),100,2);?></p>
              <p class="img-list-info">所属分类：<a href="<?php echo $mydecms['weburl'];?>/<?php echo $class_arr[$row['dafenglei']][4] <>"" ? $class_arr[$row['dafenglei']][4] : 'class.php?id='.$row['dafenglei'];?>"><?php echo $class_arr[$row['dafenglei']][1];?></a> 发布日期：<?php echo date("Y-m-d",strtotime($row['date']));?> TAG标签：
                <?php
        $tag = $row['tag'];
			if($tag<>""){
				$tagarr = explode(",",$tag);
...
?>
      ...
      <?php echo $mydecms['webad']['4'] <> "" ? '      <div class="right_ad top10">'.$mydecms['webad']['4'].'</div>':'';?> </div>
    <div class="clear"></div>
