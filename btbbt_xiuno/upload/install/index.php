...
if(empty($conf)) {
	exit('<h3>读取配置文件失败，请检查配置文件是否存在并且有可读权限：'.BBS_PATH.'conf/conf.php'.'</h3>');
}
...
if(!xn_writable(FRAMEWORK_TMP_PATH)) {
	header('Content-Type: text/html; charset=UTF-8');
	exit('<h3>请设置目录：'.FRAMEWORK_TMP_PATH.' 为可写以后再进行安装！<h3>');
}
	...
	if(core::gpc('FORM_HASH', 'P')) {
		...
		if($type == 'mysql') {
			...
			if(!$link) {
				...
				if(!$r) {
					$error = 'MySQL 账户权限可能受限：<span class="small">'.mysql_error().'</span>';
				} else {
					$c = array(
						// 主 MySQL Server
						'master' => array (
...
// 选择插件
	...
	if(core::gpc('formsubmit', 'P')) {
		$plugindir = core::gpc('plugindir', 'P');
		foreach($plugindir as $dir) {
			...
			if(!xn_writable($conffile)) {
				message('plugin 目录不可写，请设置文件为可写：'.$confile);
			}
			$mconf = new mconf();
			if(is_file($conffile)) {
			...
			if(!xn_writable($conffile)) {
				message('plugin 目录不可写，请设置文件权限为可写：'.$confile);
			}
			if($is_view) {
	...
	*/
	...
	echo '<script type="text/javascript">alert("安装完成，管理员账号为：admin, 初始密码：1，请登陆后进行修改！！！");</script>';
	include './footer.inc.php';
