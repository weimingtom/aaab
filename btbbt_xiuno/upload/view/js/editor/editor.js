...
// 向上查找父标签, 可以采用jquery 的 closeset: $(node).closest(pnode);
/*
...
$.editor = function(textarea, settings) {
	...
	this.init = function() {
		var s = '<div class="editor">\
				...
				<div class="menu">\
					...
					<div class="video" style="display: none">\
						<table width="540"><tr><td width="60">视频网址：</td><td width="450"><input type="text" size="64" value="http://" /></td></tr>\
		...
		var baseadd = '';
		_doc.write('<html><head>'+baseadd+'<meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/><link rel="stylesheet" href="' + _this.settings.baseurl + 'iframe.css"/></head><body spellcheck="false" style="overflow: auto;">'+$(textarea).val()+'</body></html>');
		_doc.close();
		_body = _doc.body;
		...
		$('a.link', toolbar).click(function() {
			...
			$(this).xn_menu($('.link', menu), 0, 1000000);
			setTimeout(function() {
		...
		$('a.video', toolbar).click(function() {
			...
			setTimeout(function() {
				$('.video input[type=text]', menu).focus().select();
			}, 100);
		});
	...
	this.clear_paste = function(e) {
		...
		var jdiv = $('<'+node+' class="xn-paste">\uFEFF\uFFFF</'+node+'>', _doc).appendTo(_doc.body)
		var div = jdiv[0];
		jdiv.css('top', $(_win).scrollTop());
	...
	this.save_bookmark = function() {
		...
		window.xn_bookmark = {'top' : $(_win).scrollTop(), 'rng' : rng};
	}
	...
	this.load_bookmark = function() {
		...
		if(window.xn_bookmark) {
			...
			$(_win).scrollTop(window.xn_bookmark.top);
	...
	this._focus = function() {
		if(_issource) {
			...
			if(rng.parentElement && rng.parentElement().ownerDocument !== _doc) {
				var rng = _this.create_range();//
				var cursorNode = _doc.body;
				if(is_ie) {
					rng.moveToElementText(cursorNode);
				} else {
					//var chileName = bLast ? 'lastChild' : 'firstChild';
					while(cursorNode.nodeType != 3 && cursorNode['lastChild']) {
	...
	*/
	...
	this.paste = function(s, overwrite) {
		...
		if(overwrite !== undefined) {
			if(rng.item) {
				var item = rng.item(0);
				rng = _this.create_range();
				rng.moveToElementText(item);
				rng.select();
			}
			rng.collapse(overwrite);
		}
		
		var html ='<' + (is_ie ? 'img' : 'span') + ' id="xn_temp" width="0" height="0" />';
		if(rng.insertNode) {
	...
	*/
	...
	this.fix_officeword = function(s) {
		if(s.match(/mso-cellspacing:/i) || s.match(/<v|o:\w+/i) || s.match(/<w:WordDocument>/i) || s.match(/<!{1}--\[if gte mso \d+\]>/i)) {
			...
			var allattrs = ['width', 'height', 'href', 'src', 'align', 'border', 'cellspaceing', 'cellspadding'];
			s = s.replace(/<!.*?>/img, '');
