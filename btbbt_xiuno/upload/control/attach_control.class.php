...
class attach_control extends common_control {
	...
	public function on_download() {
		...
		if(!is_file($this->conf['upload_path'].'attach/'.$attach['filename'])) {
	...
	public function on_uploadimage() {
		...
		if(isset($_FILES['Filedata']['tmp_name']) && is_file($_FILES['Filedata']['tmp_name'])) {
			...
			if($imginfo[2] == 1) {
				...
				$r = image::safe_thumb($file['tmp_name'], $aid, '.jpg', $uploadpath, 1210, 240000);	// 1210 800
			...
			$this->message('上传文件失败。'.print_r($_FILES, 1), 0);
		}
	}
