...
class db 
{
	...
	function update($table, $data, $where = '')
	{
		...
		$sqladd = "UPDATE $table SET $col ". (!empty($where) ? " WHERE $where" : '');
		$this->query($sqladd);
	...
	function insert($table, $data)
	{
		...
		$sqladd = "INSERT INTO $table ({$col['col']}) VALUES ({$col['val']})";
		//file_put_contents('1.txt', $sqladd);
		$this->query($sqladd);
	...
	function halt($message = '', $sql = '')
	{
		...
		if ($errorno == 2006 && $this->goneaway -- > 0) {
			...
			$s = date('Y-m-d H:i:s').' Error:' . $error . ' ';
			$s .= 'Errno:' . $errorno . ' ';
			$s .= 'SQL::' . $sql. "\r\n";
			
			$fp = fopen('sql.error.log', 'a+');
			fwrite($fp, $s);
			fclose($fp);
