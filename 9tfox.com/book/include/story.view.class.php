...
require_once(dirname(__FILE__).'/story.func.php');

$PubFields['templetdef'] = "/templets/default";
...
class BookView
{
	...
	function MakeFirst($inittype,$rsid)
	{
		...
		if($inittype=='catalog')
		{
			...
			unset($row);
			$this->dtp->LoadTemplet(DEDEROOT.'/book'.$PubFields['templetdef'].'/books_list.htm');
		...
		else if($inittype=='book')
		{
			...
			unset($row);
			$this->dtp->LoadTemplet(DEDEROOT.'/book'.$PubFields['templetdef'].'/books_book.htm');
			$this->Fields['title'] = $this->Fields['bookname'];
			$this->CatalogID = $this->Fields['catid'];
			if($this->Fields['status']==1)
			{
				$this->Fields['status']='已完成连载';
			}
			else
			{
				$this->Fields['status']='连载中...';
			}
			$this->Fields['position'] = $this->GetPosition($this->Fields['catid'],$this->Fields['bcatid']);
		}
		...
		else if($inittype=='content')
		{
			...
			{
				$this->dtp->LoadTemplet(DEDEROOT.'/book'.$PubFields['templetdef'].'/books_photo.htm');
			}
			else
			{
		...
		{
			$this->dtp->LoadTemplet(DEDEROOT.'/book'.$PubFields['templetdef'].'/books_index.htm');
		}
		...
		else if($inittype=='search')
		{
			...
			if($this->Keys['id']>0)
			{
				$this->CatalogID = $this->Keys['id'];
			}
			$this->dtp->LoadTemplet(DEDEROOT.'/book'.$PubFields['templetdef'].'/books_search.htm');
		}
	}
	...
	function ParseTempletsFirst()
	{
		...
		foreach($this->dtp->CTags as $tagid=>$ctag)
		{
			...
			else if($tagname=='newcontent')
			{
				...
				{
					$this->dtp->Assign($tagid, $this->GetNewContentLink($bid,$ctag->GetInnerText()));
				}
			}
	...
	function ParseDmFields($mtype,$pageno)
	{
		...
		foreach($this->dtp->CTags as $tagid=>$ctag)
		{
			...
			if($tagname=='pagelist')
			{
				if($mtype=='dm')
				{
					$this->dtp->Assign($tagid,$this->GetPageListDM($ctag->GetAtt('listsize'),$ctag->GetAtt('listitem')) );
				}
			}
		}
	}
	...
	function GetNewContentLink($bid,$innertext='')
	{
		...
		if(empty($innertext))
		{
			$innertext = "<a href='[field:url/]'>[field:title/]</a>";
		}
		if($this->Fields['booktype']==1)
		{
		...
		while($row = $this->dsql->GetArray())
		{
			...
			if(is_array($ndtp->CTags))
			{
				foreach($ndtp->CTags as $tagid=>$ctag)
				{
					...
					{
						$this->dsql->SetQuery("Select id,title,sortid,bigpic From #@__story_content where chapterid='{$row['id']}' order by sortid asc");
	...
	function GetPosition($catid,$bcatid)
	{
		...
		if(is_array($row))
		{
			$oklink  = "<a href='{$cfg_cmspath}/book/list.php?id={$row['id']}'>{$row['classname']}</a>";
		}
		$row = $this->dsql->GetArray();
		if(is_array($row))
		{
			$oklink  .= " ".trim($cfg_list_symbol)." {$row['classname']}";
		}
		return $oklink;
	}
	...
	function GetPreNext($gtype)
	{
		if(count($this->PreNext)==0)
		{
			...
			else
			{
				...
				{
					$this->PreNext['pre']['link']="story.php?id=".$row['id'];
				}
			}
			...
			else
			{
				...
				{
					$this->PreNext['next']['link']="story.php?id=".$row['id'];
				}
			}
		}
		if($gtype=='prelink')
		{
			return "<a href='{$this->PreNext['pre']['link']}'>$this->PreNext['pre']['title']</a>";
		}
		else if($gtype=='nextlink')
		{
			return "<a href='{$this->PreNext['next']['link']}'>$this->PreNext['next']['title']</a>";
		}
		else if($gtype=='preurl')
		{
			return $this->PreNext['pre']['link'];
		}
		else if($gtype=='nexturl')
		{
			return $this->PreNext['next']['link'];
		}
		else
		{
			return "";
	...
	function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
	{
		...
		for($j;$j<=$total_list;$j++)
		{
			if($j==$this->PageNo)
			{
				$listdd.= "<dd><span>$j</span></dd>\r\n";
			}
			else
			{
				$listdd.="<dd><a href='".$purl."PageNo=$j'>[".$j."]</a></dd>\r\n";
			}
		}
