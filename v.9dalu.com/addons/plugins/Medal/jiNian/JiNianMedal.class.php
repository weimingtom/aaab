<?php
class JiNianMedal extends BaseMedal {
	
	protected $_is_ji_nian;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isJiNian($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isJiNian($uid) ) {
		...
		$result['next_level_time']	= $this->__isJiNian($uid) ? '0' : '绑定新浪';
		return $result;
	}
	...
	private function __isJiNian($uid) {
		if (!isset($this->_is_ji_nian)) {
			$map['uid']					= $uid;
			
			$this->_is_ji_nian = M('weibo')->where($map)->find() ? true : false;
		}
		return $this->_is_ji_nian;
	}
