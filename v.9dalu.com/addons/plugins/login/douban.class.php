...
require_once SITE_PATH . '/addons/plugins/login/douban/OAuth.php';
require_once SITE_PATH . '/addons/plugins/login/douban/doubanOAuth.php';
...
class douban {
	...
	public function checkUser($oauth_token) {
		if ( empty($this->_douban_key) || empty($this->_douban_secret) || empty($oauth_token) || empty($_SESSION['douban']['request_token']['oauth_token_secret']) )
			return false;
			
		$client = new DoubanOAuth($this->_douban_key, $this->_douban_secret, $oauth_token, $_SESSION['douban']['request_token']['oauth_token_secret']);
