	...
	<body>
		<div class="so_main">
			...
			if (!empty($_POST['cmd_delete']) && $_POST['cmd_delete']=="删除"){
				foreach ($_POST['checkbox'] as $label_id){
					mysql_query("DELETE FROM ts_plugins_label WHERE label_id='".$label_id."'");
				}
			}
			...
			$SQL=$SQL."WHERE ";
			$SQL=$SQL."		INSTR(label_name,'".$_POST['label_name']."')>0 ";
			$COUNT_RECORD=0;
			...
			<FORM NAME="FRM" METHOD="POST" ACTION="index.php">
				<div class="Toolbar_inbox">
					...
					标签：<input type="text" name="label_name" value="<?php echo $_POST['label_name']?>" style="width:100px;"/>
				...
				<div class="list">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<th class="line_l">标签</th>
							<th class="line_l">热门</th>
						...
						?>
							<tr overstyle='on' id="credit_<?php echo $RS['id']?>">
								<td><input type="checkbox" name="checkbox[]" id="checkbox[]" value="<?php echo $RS['label_id']?>"></td>
								<td><?php echo $RS['label_id']?></td>
								<td><?php echo $RS['label_name']?></td>
								<td>
									<?php
									if ($RS['label_hot']==1)
										echo '是';
								...
								<td><?php echo $RS['label_seq']?></td>
								<td>
								<?php if ($_SESSION['mid']!=1){ ?>
									<span style="color:grey;">编辑</span>

									<?php }else{ ?>
									<a href="edit.php?label_id=<?php echo $RS['label_id']?>">编辑</a> 
									<?php
									}
									?>
				...
				<div class="Toolbar_inbox">
					...
					<a class="btn_a" href="edit.php?label_id=0"><span>新增</span></a>
					<?php
									}
									?>
				</div>
